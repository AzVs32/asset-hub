...
//! routes through these helpers so the internal namespace layout has one source of truth instead
//! of being re-concatenated across `content`, `command`, and `upload`.
...
pub(super) fn replacement_backup_key(
    ...
    Ok(StorageKey::internal(format!("content-backups/{id}"))?)
}
...
pub(super) fn deletion_key(id: ResourceId) -> Result<StorageKey, CoreError> {
    Ok(StorageKey::internal(format!("deletions/{id}"))?)
}
