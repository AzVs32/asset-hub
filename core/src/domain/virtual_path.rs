...
use crate::domain::{EntryName, VirtualRelativePath};
...
impl VirtualPath {
    ...
    pub fn join_segment(&self, segment: impl AsRef<str>) -> Result<Self, CoreError> {
        let name = EntryName::try_from(segment.as_ref())?;
        self.join_name(&name)
    }

    /// Appends an already validated entry name to this path.
    pub fn join_name(&self, name: &EntryName) -> Result<Self, CoreError> {
