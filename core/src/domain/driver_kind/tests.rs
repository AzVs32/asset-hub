...
fn driver_kind_is_a_stable_registry_identifier() {
    ...
    assert!(matches!(
        ...
        CoreError::DriverKind(DriverKindError::InvalidCharacter { character }) if character == 'M'
    ));
}
...
fn driver_kind_rejects_empty_and_oversized_identifiers() {
    assert!(matches!(
        ...
        CoreError::DriverKind(DriverKindError::Empty)
    ));

    let oversized = "a".repeat(65);
    assert!(matches!(
        DriverKind::try_from(oversized.as_str()).unwrap_err(),
        CoreError::DriverKind(DriverKindError::TooLong { length, max }) if length == 65 && max == 64
    ));
}
