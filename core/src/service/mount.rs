...
use crate::domain::{Mount, ResolvedMount};
...
impl MountService {
    ...
    pub fn new(mounts: Vec<Mount>) -> Result<Self, CoreError> {
        Self::validate_mounts(&mounts)?;
        Ok(Self { mounts })
    }

    fn validate_mounts(mounts: &[Mount]) -> Result<(), MountServiceError> {
        let mut ids = HashSet::new();
        let mut paths = HashSet::new();

        for mount in mounts {
            if !ids.insert(mount.id()) {
                return Err(MountServiceError::DuplicateMountId(mount.id()));
            }
            if !paths.insert(mount.virtual_path()) {
                return Err(MountServiceError::DuplicateMountPath(
