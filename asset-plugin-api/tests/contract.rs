...
fn request_and_output_wire_shapes_match_the_current_goldens() {
    ...
    assert_golden_round_trip::<PluginResourceActionRequest>(include_str!(
        "fixtures/resource-action-request-inline-v3.json"
    ));
    assert_golden_round_trip::<PluginResourceActionRequest>(include_str!(
        "fixtures/resource-action-request-reference-v3.json"
    ));
    assert_golden_round_trip::<PluginResourceActionOutput>(include_str!(
        "fixtures/resource-action-output-v3.json"
    ));
    assert_golden_round_trip::<PluginResourceActionOutput>(include_str!(
        "fixtures/resource-action-output-download-v3.json"
    ));
    assert_golden_round_trip::<PluginActionFailure>(include_str!(
        "fixtures/action-failure-v3.json"
    ));
    assert_golden_round_trip::<DirectoryPageRequest>(include_str!(
        "fixtures/directory-page-request-v3.json"
    ));
}
...
fn committed_language_neutral_specs_are_current() {
    ...
    let expected: Value =
        serde_json::from_slice(&std::fs::read(root.join("contract-v3.json")).unwrap()).unwrap();
    assert_eq!(
        expected,
...
fn context_specific_encodings_reject_invalid_wire_combinations() {
    ...
    let mut output: Value =
        serde_json::from_str(include_str!("fixtures/resource-action-output-v3.json")).unwrap();
