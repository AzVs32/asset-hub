...
use asset_plugin_api::protocol::{
    PluginActionFailure, PluginDiagnostic, PluginDiagnosticSeverity, diagnostic_codes as codes,
};

#[test]
fn plugin_diagnostic_codes_map_to_stable_http_statuses() {
    assert_eq!(
        plugin_status(asset_plugin_api::protocol::diagnostic_codes::INVALID_INPUT),
        StatusCode::BAD_REQUEST
    );
    assert_eq!(
        plugin_status(asset_plugin_api::protocol::diagnostic_codes::PERMISSION_DENIED),
        StatusCode::FORBIDDEN
    );
    assert_eq!(
        plugin_status(asset_plugin_api::protocol::diagnostic_codes::CONTENT_LIMIT_EXCEEDED),
        StatusCode::PAYLOAD_TOO_LARGE
    );
    assert_eq!(
        plugin_status(asset_plugin_api::protocol::diagnostic_codes::TIMEOUT),
