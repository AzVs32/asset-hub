...
fn virtual_path_enforces_path_and_segment_byte_limits() {
    ...
    assert!(matches!(
        ...
        VfsError::Namespace(NamespaceError::EntryNameTooLong { length, max }) if length == MAX_SEGMENT_BYTES + 1 && max == MAX_SEGMENT_BYTES
    ));
    ...
    assert!(matches!(
        ...
        VfsError::Namespace(NamespaceError::VirtualPathTooLong { length, max }) if length == MAX_PATH_BYTES + 2 && max == MAX_PATH_BYTES
    ));

    let oversizedriver_path = format!("/{}", vec![segment; 17].join("/"));
    assert!(matches!(
        VirtualPath::try_from(oversizedriver_path.as_str()).unwrap_err(),
        VfsError::Namespace(NamespaceError::VirtualPathTooLong { length, max }) if length == oversizedriver_path.len() && max == MAX_PATH_BYTES
    ));
}
