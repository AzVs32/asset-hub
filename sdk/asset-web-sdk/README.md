...
- `@asset-hub/asset-web-sdk` for frame clients and all public types;
- `@asset-hub/asset-web-sdk/contract` for transport-free constants and Browser Frame types used
  by Host implementations;
- `@asset-hub/asset-web-sdk/global` for resolving the self-contained IIFE asset in packaging
  tools.
...
A plugin with a plain `index.html` can copy `dist/asset-web-sdk.global.js` into its verified Web
...
```ts
import { connectAssetHubDirectoryFrame } from "@asset-hub/asset-web-sdk";
...
const readme = await host.viewResource("resource-id");
await host.editResource("resource-id");
```
