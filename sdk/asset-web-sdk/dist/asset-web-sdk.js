...
}, z = "asset-hub.plugin-api@4", B = "asset-hub.plugin-frame.resource@4", V = "asset-hub.plugin-frame.directory@4", H = [
	"thumbnail",
	"view",
	"edit"
...
var W = 1e4, G = 3e4;
async function K(e = {}) {
	if (window.parent === window) throw Error("Asset Web SDK must run inside a plugin frame.");
	let t = Z(e.connectionTimeoutMs, W, "connectionTimeoutMs"), n = Z(e.callTimeoutMs, G, "callTimeoutMs"), r = L({
		messenger: new R({
...
async function q(e = {}) {
	if (window.parent === window) throw Error("Asset Web SDK must run inside a directory plugin frame.");
	let t = Z(e.connectionTimeoutMs, W, "connectionTimeoutMs"), n = Z(e.callTimeoutMs, G, "callTimeoutMs"), r = L({
		messenger: new R({
			remoteWindow: window.parent,
			allowedOrigins: ["*"]
...
function J({ client: e, frame: t, resourceId: n, output: r, connectionTimeoutMs: i }) {
	...
	if (a.plugin_api !== "asset-hub.plugin-api@4") throw Error(`Unsupported Plugin Frame API: ${a.plugin_api}`);
	...
	let c = Z(i, W, "connectionTimeoutMs"), l = L({
		messenger: new R({
			remoteWindow: s,
			allowedOrigins: ["*"]
		}),
...
function Y(e) {
	let [t] = e.split(/[?#]/, 1);
	if (!t || !/^\/plugins\/[a-z0-9._-]+\//.test(t) || X(t)) throw Error("The Resource Action returned an invalid plugin frame URL.");
	return e;
}
function X(e) {
	try {
